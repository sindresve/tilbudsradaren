...
import { X, Trash2, ShoppingCart, ChevronsRight, TriangleAlert, FileJson, Upload, PackagePlus, ListPlus, Download, FileUp, Import } from "lucide-react";
...
function formatPrice(value: number | null) {
  if (value === null) return "Ingen pris";
...
export default function ShoppingListSidebar({
  ...
  const saved = products.reduce(
    (sum, p) =>
      sum + ((p.old_price ?? 0) - (p.current_price ?? 0)),
    0
  );

  const hasMissingPrice = products.some(
    (p) => p.current_price === null || p.old_price === null
    );
